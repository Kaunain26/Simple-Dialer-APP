...
import com.simplemobiletools.smsmessenger.databinding.ActivityMain2Binding
...
class MainActivity2 : SimpleActivity() {
    ...
    private fun refreshMenuItems() {
        binding.mainMenu.getToolbar().menu.apply {
            ...
            findItem(R.id.show_recycle_bin).isVisible = config_sms.useRecycleBin
            findItem(R.id.show_archived).isVisible = config_sms.isArchiveAvailable
        }
    }
    ...
    private fun storeStateVariables() {
        ...
        storedFontSize = config_sms.fontSize
    }
    ...
    private fun checkShortcut() {
        val appIconColor = config_sms.appIconColor
        if (isNougatMR1Plus() && config_sms.lastHandledShortcutColor != appIconColor) {
            val newConversation = getCreateNewContactShortcut(appIconColor)

            val manager = getSystemService(ShortcutManager::class.java)
            try {
                manager.dynamicShortcuts = listOf(newConversation)
                config_sms.lastHandledShortcutColor = appIconColor
            } catch (ignored: Exception) {
            }
        }
    }
    ...
    private fun updateFilteredConversations(cached: Boolean = false) {
        ...
        if (cached && config_sms.appRunCount == 1) {
    ...
    private fun filterConversations(conversations: List<Conversation>): List<Conversation> {
        ...
        return when (currentFilter) {
            ...
            ConversationFilter.OTP -> conversations.filter { conversation ->
                conversation.isOtp() && !isNumberBlocked(conversation.phoneNumber, blockedNumbers)
            }

            ConversationFilter.PROMOTIONAL -> conversations.filter { conversation ->
                conversation.isPromotional() && !isNumberBlocked(conversation.phoneNumber, blockedNumbers)
            }
